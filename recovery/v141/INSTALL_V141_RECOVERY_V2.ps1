param([string]$Root=(Join-Path ([Environment]::GetFolderPath('MyDocuments')) 'Domlight'))
$ErrorActionPreference='Stop'
Add-Type -AssemblyName System.Windows.Forms
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12

function Info([string]$s){[Windows.Forms.MessageBox]::Show($s,'Domlight v141 Recovery','OK','Information')|Out-Null}
function Fail([string]$s){[Windows.Forms.MessageBox]::Show($s,'Domlight v141 Recovery','OK','Error')|Out-Null}
...
try{
 ...
 foreach($n in @('GmailApi.ps1','Recipients.ps1')){$p=Join-Path $Root $n;if(-not(Test-Path $p)){throw "Не найден сохранённый $n"};if(-not(ParseOk $p)){throw "Parser error: $n"}}
 "Legacy Mailing: $oldMail"|Add-Content $log -Encoding UTF8;"Legacy ConnectionSettings: $oldConn"|Add-Content $log -Encoding UTF8
 $managed=@($pins|ForEach-Object{$_[0]})+@('Mailing.ps1','ConnectionSettings.ps1','UpdateFromGitHub.ps1','SelfCheck.ps1','VERSION.txt');$had=@{}
 ...
 try{
  ...
  $self=@(
   ...
   '$m=Get-Content (Join-Path $Root ''MeterStatus.ps1'') -Raw;if($m-match''/meter/value''){$e+=''Meter send enabled''}',
 ...
 exit 0
}catch{if($log){"ERROR: $($_.Exception.Message)"|Add-Content $log -Encoding UTF8};Fail($_.Exception.Message);Write-Host "ERROR: $($_.Exception.Message)" -ForegroundColor Red;exit 1}
