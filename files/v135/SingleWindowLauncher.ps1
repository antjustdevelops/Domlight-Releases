...
# Separate mutex namespace for each Domlight installation folder.
    ...
    try {
        if(Test-Path -LiteralPath $pidFile){
            ...
            if($oldPid-gt0){
                ...
                if($p){
                    try{[Microsoft.VisualBasic.Interaction]::AppActivate($oldPid)|Out-Null}catch{}
                }
            }
        }
    } catch {}
...
try {
    ...
    # ESC closes the currently active child form in this launcher process.
    # The main Domlight dashboard is not launched through this wrapper, so ESC never closes it.
