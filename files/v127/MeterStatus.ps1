...
foreach ($spec in @(
    ...
    if ($spec.Name -eq 'Address') { $column.AutoSizeMode = 'Fill'; $column.MinimumWidth = 330 }
...
function Show-ObjectDialog([string]$Account) {
    ...
    $selectColumn.Width = 75
    [void]$meterGrid.Columns.Add($selectColumn)

    foreach ($spec in @(
        @{Name='Type';Header='Счётчик';Width=135;ReadOnly=$true},
        @{Name='Number';Header='№ прибора';Width=135;ReadOnly=$true},
        @{Name='LastValue';Header='Последнее';Width=90;ReadOnly=$true},
        @{Name='Unit';Header='Ед.';Width=55;ReadOnly=$true},
        @{Name='LastDate';Header='Передавали';Width=100;ReadOnly=$true},
        @{Name='MonthStatus';Header='Статус';Width=105;ReadOnly=$true},
        @{Name='NewValue';Header='Новое показание';Width=120;ReadOnly=$false},
        @{Name='Verification';Header='Поверка';Width=120;ReadOnly=$true}
    )) {
        $column = New-Object Windows.Forms.DataGridViewTextBoxColumn
        $column.Name = $spec.Name
        $column.HeaderText = $spec.Header
