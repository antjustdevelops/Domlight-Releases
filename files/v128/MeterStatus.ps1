...
function Parse-Meters([string]$Html, [string]$Account, [string]$Address) {
    ...
    if ($allStart -ge 0 -and $waterStart -gt $allStart) { $scope = $Html.Substring($allStart, $waterStart - $allStart) }
    else { $scope = $Html }
    ...
    for ($i = 0; $i -lt $positions.Count; $i++) {
        $start = $positions[$i]
        if ($i + 1 -lt $positions.Count) { $end = $positions[$i + 1] }
        else { $end = $scope.Length }
        ...
        if ($headM.Success) { $headText = Clean-Text $headM.Groups[1].Value }
        else { $headText = '' }
        $type = $headText
        $number = ''
...
function Build-ObjectRows {
    ...
    foreach ($group in @($script:AllMeters | Group-Object Account)) {
        ...
        if ($pending -eq 0) { $state = 'Всё передано' }
        else { $state = 'Нужно передать: ' + $pending }
...
$form.ClientSize = New-Object Drawing.Size(1250, 760)
$form.MinimumSize = New-Object Drawing.Size(980, 560)
...
$summary.Size = New-Object Drawing.Size(1185, 25)
...
foreach ($spec in @(
    @{ Name='Address'; Header='Объект'; Weight=44; Min=320 },
    @{ Name='Account'; Header='Лицевой счёт'; Weight=16; Min=130 },
    @{ Name='MeterCount'; Header='Счётчиков'; Weight=10; Min=85 },
    @{ Name='Status'; Header='Статус месяца'; Weight=18; Min=145 },
    @{ Name='DraftCount'; Header='Готово к отправке'; Weight=12; Min=120 }
)) {
    ...
    $column.MinimumWidth = $spec.Min
    [void]$grid.Columns.Add($column)
}

$btnOpen = New-Object Windows.Forms.Button
$btnOpen.Text = 'Открыть объект'
$btnOpen.Size = New-Object Drawing.Size(170, 38)
$btnOpen.Location = New-Object Drawing.Point(1060, 697)
...
function Refresh-Grid {
    ...
    foreach ($item in $script:ObjectRows) {
        switch ($script:CurrentFilter) {
            'need' { $filterOk = $item.PendingCount -gt 0 }
            'done' { $filterOk = $item.PendingCount -eq 0 }
        ...
        $row.Cells['Account'].Value = $item.Account
        $row.Cells['MeterCount'].Value = $item.MeterCount
        $row.Cells['Status'].Value = $item.Status
        $row.Cells['DraftCount'].Value = $item.DraftCount
        $row.Tag = $item.Account
        if ($item.PendingCount -eq 0) { $row.Cells['Status'].Style.ForeColor = [Drawing.Color]::DarkGreen }
        else { $row.Cells['Status'].Style.ForeColor = [Drawing.Color]::DarkRed }
    }
}
...
function Show-ObjectDialog([string]$Account) {
    ...
    $dialog.ClientSize = New-Object Drawing.Size(1220, 580)
    $dialog.MinimumSize = New-Object Drawing.Size(1040, 480)
    ...
    $addressLabel.Size = New-Object Drawing.Size(1175, 30)
    $addressLabel.Anchor = 'Top,Left,Right'
    ...
    $accountLabel.Size = New-Object Drawing.Size(1160, 24)
    ...
    $meterGrid.Size = New-Object Drawing.Size(1184, 420)
    ...
    $meterGrid.AutoSizeColumnsMode = 'Fill'
    $meterGrid.ScrollBars = 'Vertical'
    $dialog.Controls.Add($meterGrid)

    $selectColumn = New-Object Windows.Forms.DataGridViewCheckBoxColumn
    $selectColumn.Name = 'Selected'
    $selectColumn.HeaderText = 'Передать'
    $selectColumn.ReadOnly = $false
    $selectColumn.AutoSizeMode = 'Fill'
    $selectColumn.FillWeight = 8
    $selectColumn.MinimumWidth = 70
    [void]$meterGrid.Columns.Add($selectColumn)

    foreach ($spec in @(
        @{Name='Type';Header='Счётчик';Weight=16;Min=125;ReadOnly=$true},
        @{Name='Number';Header='№ прибора';Weight=15;Min=125;ReadOnly=$true},
        @{Name='LastValue';Header='Последнее';Weight=11;Min=90;ReadOnly=$true},
        @{Name='Unit';Header='Ед.';Weight=6;Min=55;ReadOnly=$true},
        @{Name='LastDate';Header='Передавали';Weight=11;Min=95;ReadOnly=$true},
        @{Name='MonthStatus';Header='Статус';Weight=12;Min=100;ReadOnly=$true},
        @{Name='NewValue';Header='Новое показание';Weight=13;Min=115;ReadOnly=$false},
        @{Name='Verification';Header='Поверка';Weight=12;Min=105;ReadOnly=$true}
    )) {
        $column = New-Object Windows.Forms.DataGridViewTextBoxColumn
        $column.Name = $spec.Name
        $column.HeaderText = $spec.Header
        $column.ReadOnly = $spec.ReadOnly
        $column.AutoSizeMode = 'Fill'
        $column.FillWeight = $spec.Weight
        $column.MinimumWidth = $spec.Min
    ...
    foreach ($meter in $meters) {
        ...
        $rowIndex = $meterGrid.Rows.Add()
        $row = $meterGrid.Rows[$rowIndex]
        $row.Tag = $meter.MeterId
        $row.Cells['Selected'].Value = $selected
        $row.Cells['Type'].Value = $meter.Type
        $row.Cells['Number'].Value = $meter.Number
        $row.Cells['LastValue'].Value = $meter.LastValue
        $row.Cells['Unit'].Value = $meter.Unit
        $row.Cells['LastDate'].Value = $meter.LastDate
        $row.Cells['MonthStatus'].Value = $meter.MonthStatus
        $row.Cells['NewValue'].Value = $newValue
        $row.Cells['Verification'].Value = $meter.Verification
        if ($meter.MonthStatus -eq 'Передано') {
            $row.Cells['MonthStatus'].Style.ForeColor = [Drawing.Color]::DarkGreen
            $row.Cells['Selected'].ReadOnly = $true
            $row.Cells['Selected'].Style.BackColor = [Drawing.Color]::Gainsboro
            $row.Cells['NewValue'].ReadOnly = $true
            $row.Cells['NewValue'].Style.BackColor = [Drawing.Color]::Gainsboro
        }
        else {
            $row.Cells['MonthStatus'].Style.ForeColor = [Drawing.Color]::DarkRed
        }
    }

    $btnSave = New-Object Windows.Forms.Button
    $btnSave.Text = 'Сохранить черновик'
    $btnSave.Size = New-Object Drawing.Size(180, 38)
    $btnSave.Location = New-Object Drawing.Point(815, 522)
    $btnSave.Anchor = 'Bottom,Right'
    $dialog.Controls.Add($btnSave)

    $btnClose = New-Object Windows.Forms.Button
    $btnClose.Text = 'Закрыть'
    $btnClose.Size = New-Object Drawing.Size(125, 38)
    $btnClose.Location = New-Object Drawing.Point(1015, 522)
...
$openSelected = {
    ...
    $account = [string]$grid.SelectedRows[0].Tag
