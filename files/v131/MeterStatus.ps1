...
function Parse-Meters([string]$Html,[string]$Account,[string]$Address) {
    ...
    for ($i=0; $i -lt $positions.Count; $i++) {
        $start = $positions[$i]
        if ($i+1 -lt $positions.Count) { $end = $positions[$i+1] } else { $end = $scope.Length }
...
# Main window starts compact and is resized after the object count is known.
...
function Set-MainCompactHeight {
    $visibleRows=[Math]::Min([Math]::Max($script:ObjectRows.Count,1),12)
    $gridHeight=28+($visibleRows*24)+3
    $clientHeight=126+$gridHeight+78
    if($clientHeight -lt 330){$clientHeight=330}
    $form.ClientSize=New-Object Drawing.Size($form.ClientSize.Width,$clientHeight)
    $grid.Size=New-Object Drawing.Size($grid.Width,$gridHeight)
    $btnOpen.Location=New-Object Drawing.Point($btnOpen.Left,$clientHeight-63)
}
...
function Show-ObjectDialog([string]$Account) {
    ...
    $visibleRows=[Math]::Min([Math]::Max($meters.Count,1),8)
    $gridHeight=28+($visibleRows*24)+3
    $dialogHeight=82+$gridHeight+78
    if($dialogHeight -lt 270){$dialogHeight=270}

    $dialog=New-Object Windows.Forms.Form
    $dialog.Text='Показания — '+$meters[0].Address;$dialog.StartPosition='CenterParent';$dialog.ClientSize=New-Object Drawing.Size(1220,$dialogHeight);$dialog.MinimumSize=New-Object Drawing.Size(1040,270);$dialog.Font=New-Object Drawing.Font('Segoe UI',10);$dialog.KeyPreview=$true;$dialog.MaximizeBox=$true
    ...
    $buttonY=$dialogHeight-58
    $btnSave=New-Object Windows.Forms.Button;$btnSave.Text='Сохранить черновик';$btnSave.Size=New-Object Drawing.Size(180,38);$btnSave.Location=New-Object Drawing.Point(815,$buttonY);$btnSave.Anchor='Bottom,Right';$dialog.Controls.Add($btnSave)
    $btnClose=New-Object Windows.Forms.Button;$btnClose.Text='Закрыть';$btnClose.Size=New-Object Drawing.Size(125,38);$btnClose.Location=New-Object Drawing.Point(1015,$buttonY);$btnClose.Anchor='Bottom,Right';$dialog.Controls.Add($btnClose)
