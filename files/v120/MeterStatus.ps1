...
function Load-WebSession {
    ...
    # IMPORTANT: identical array handling to the working Domlight.ps1.
...
function Parse-Meters([string]$Html, [string]$Account, [string]$Address) {
    $start = $Html.IndexOf('id="tab_meter_all"')
    if ($start -lt 0) { $start = $Html.IndexOf("id='tab_meter_all'") }
    $end = $Html.IndexOf('id="tab_meter_water"')
    if ($end -lt 0) { $end = $Html.IndexOf("id='tab_meter_water'") }
    if ($start -ge 0 -and $end -gt $start) { $scope = $Html.Substring($start, $end - $start) } else { $scope = $Html }

    $inputs = [regex]::Matches($scope, '<input[^>]+class=["''][^"'']*meter-input[^"'']*["''][^>]*>', 'IgnoreCase')
    $rows = @()
    $seen = @{}

    foreach ($im in $inputs) {
        $tag = $im.Value
        ...
        $meterId = $idM.Groups[1].Value
        if ($seen.ContainsKey($meterId)) { continue }
        $seen[$meterId] = $true

        $pos = $im.Index
        $left = [Math]::Max(0, $pos - 1800)
        $len = [Math]::Min(2600, $scope.Length - $left)
        $context = $scope.Substring($left, $len)

        $heads = [regex]::Matches($context, '(?is)<h2[^>]*>(.*?)</h2>')
        $headText = ''
        if ($heads.Count -gt 0) { $headText = Clean-Text $heads[$heads.Count - 1].Groups[1].Value }
        $number = ''
        $type = $headText
        ...
        $date = ''
        $dateM = [regex]::Match($context, 'Передавали\s+(\d{2}\.\d{2}\.\d{4})', 'IgnoreCase')
        if ($dateM.Success) { $date = $dateM.Groups[1].Value }

        $previous = ''
        $prevM = [regex]::Match($context, 'Показания\s+прошлого\s+периода\s+([0-9.,]+)', 'IgnoreCase')
        if ($prevM.Success) { $previous = $prevM.Groups[1].Value.Replace(',', '.') }

        $verification = ''
        $verM = [regex]::Match($context, 'title=["'']Поверка:\s*([^"'']*)["'']', 'IgnoreCase')
...
$status.Size = New-Object Drawing.Size(1300,25)
...
$specs = @(
    @{N='Account';H='Лицевой счёт';W=125},
    @{N='Address';H='Адрес';W=390},
    @{N='Type';H='Счётчик';W=150},
    @{N='Number';H='№ прибора';W=145},
    @{N='Value';H='Текущее';W=90},
    @{N='Previous';H='Прошлый период';W=115},
    @{N='Unit';H='Ед.';W=70},
    @{N='LastDate';H='Передавали';W=105},
    @{N='Verification';H='Поверка';W=120}
)
foreach ($s in $specs) {
    $c = New-Object Windows.Forms.DataGridViewTextBoxColumn
    $c.Name=$s.N; $c.DataPropertyName=$s.N; $c.HeaderText=$s.H; $c.Width=$s.W
    if ($s.N -eq 'Address') { $c.AutoSizeMode='Fill'; $c.MinimumWidth=300 }
...
$form.Add_Shown({
    try {
        ...
        foreach ($acc in $accounts) {
            ...
            foreach ($row in @(Parse-Meters ([string]$meterPage.Content) ([string]$acc.Account) $address)) {
                [void]$rows.Add($row)
            }
        }

        $grid.DataSource = $rows
        $status.Text = "Готово. Лицевых счетов проверено: $checked. Счётчиков найдено: $($rows.Count). Только чтение — передача показаний отключена."
