...
function Show-ObjectDialog([string]$Account) {
    ...
    Enable-MeterGridSelectionRules -Grid $meterGrid
...
$form.Add_Shown({
    try{
        ...
        $script:AllMeters=@($meters);[void](Remove-StaleMeterDrafts -Drafts $script:Drafts -Meters $script:AllMeters);Export-MeterDraftStore -Drafts $script:Drafts -Path $DraftsFile;Build-ObjectRows;Set-MainCompactHeight;Refresh-Grid
