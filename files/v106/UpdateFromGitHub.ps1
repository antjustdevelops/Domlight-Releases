...
if ($CurrentVersion -match '(?i)(DEV|RELEASE CANDIDATE|\bRC\b)') {
    [Windows.Forms.MessageBox]::Show('Автоматическое production-обновление отключено в тестовой сборке Release Candidate.','Domlight RC','OK','Information') | Out-Null
    exit 0
}
...
try {
    ...
    # Keep visible version labels in sync with the manifest even for a version-only test update.
    ...
    # Do NOT launch the new Domlight window here.
    # The currently running main window receives exit code 10, hides/closes itself first,
    # and only then starts the updated version. This prevents two Domlight windows
