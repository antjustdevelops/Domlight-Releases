...
function Read-LastCheck {
    ...
    try {
        ...
        $accounts = "—"
        if ([string]$x.Details -match 'Проверено лицевых счетов:\s*(\d+)') {
            $accounts = $matches[1]
        }
        return [pscustomobject]@{
            CheckedAt = [string]$x.CheckedAt
            Result = [string]$x.Result
            NewCount = [string]$x.NewCount
        ...
        return [pscustomobject]@{
            ...
            Result = "Ошибка"
            NewCount = 0
            Details = "Не удалось прочитать статус"
            Accounts = "—"
        }
    }
}
...
$form.Text = "Domlight"
$form.Size = New-Object Drawing.Size(720, 970)
...
$cardAccounts = Add-Card "Лицевых счетов" 30 $state.Accounts "по последней проверке"
$cardLast = Add-Card "Последняя проверка" 255 (Short-Date $state.CheckedAt) (Short-Time $state.CheckedAt) 15
$cardNew = Add-Card "Новых квитанций" 480 ([string]$state.NewCount) $state.Result
...
$btnAccounts = Add-Button "Лицевые счета / статус" 30 474 655 ([Drawing.Color]::FromArgb(14,116,144)) {
    try { Run-Hidden (Join-Path $Root "AccountStatus.ps1") } catch {
        [Windows.Forms.MessageBox]::Show("Не удалось открыть лицевые счета:`r`n$($_.Exception.Message)","Domlight","OK","Error") | Out-Null
    }
}

$btnResetArchive = Add-Button "↻  Пересоздать архив квитанций" 30 544 655 ([Drawing.Color]::FromArgb(100,116,139)) {
    Reset-ReceiptArchive
}

$btnMailing = Add-Button "✉  Рассылка квитанций" 30 614 655 ([Drawing.Color]::FromArgb(124,58,237)) {
    try {
...
$infoPanel.Location = New-Object Drawing.Point(30,684)
...
$info.Text = "Готово к работе.`r`nПоследняя проверка: $($state.Result)"
...
function Refresh-Dashboard {
    ...
    $cardLast.Value.Text = Short-Date $s.CheckedAt
    $cardLast.Small.Text = Short-Time $s.CheckedAt
    $cardNew.Value.Text = [string]$s.NewCount
    $cardNew.Small.Text = [string]$s.Result
    $proxyStatus.Text = Proxy-Status
}
...
$timer.Add_Tick({
    ...
    if ($latestProgress.Count -gt 0) {
        try {
            ...
            if ($p.Downloaded -gt 0) {
                $parts += ("скачано новых: " + $p.Downloaded)
            }
    ...
    if (Test-Path $CheckDoneFile) {
        ...
        if ($s.Result -eq "Успешно") {
            ...
            $info.Text = "Успешно.`r`n" + [string]$s.Details
            $done.ForeColor = [Drawing.Color]::Green
        } else {
            $done.Text = "✖ Проверка завершена с ошибкой: " + [string]$s.Details
            $info.Text = "Ошибка текущей проверки.`r`n" + [string]$s.Details
            $done.ForeColor = [Drawing.Color]::DarkRed
        }
...
$btnUpdate.Text = "Проверить обновление"
