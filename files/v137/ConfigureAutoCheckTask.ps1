...
$Script = Join-Path $Root 'RunAutoCheckHidden.ps1'
$DataDir = Join-Path $Root 'data'
$MigrationFlag = Join-Path $DataDir 'task_settings_v106.flag'
New-Item -ItemType Directory -Force -Path $DataDir | Out-Null
# Clean up old test tasks from pre-release builds.
try { & schtasks.exe /Delete /TN 'Domlight DEV Auto Check' /F *> $null } catch {}
try { & schtasks.exe /Delete /TN 'Domlight RC Auto Check' /F *> $null } catch {}
