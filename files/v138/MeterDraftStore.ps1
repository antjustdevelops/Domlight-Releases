...
function Import-MeterDraftStore {
    ...
    try {
        ...
        foreach ($item in @($payload.drafts)) {
            ...
            $meterId = ([string]$item.meterId).Trim()
...
function Export-MeterDraftStore {
    param(
        [Parameter(Mandatory=$true)][hashtable]$Drafts,
...
function Remove-StaleMeterDrafts {
    param(
        [Parameter(Mandatory=$true)][hashtable]$Drafts,
        [Parameter(Mandatory=$true)][array]$Meters
    )

    $valid = @{}
    foreach ($meter in @($Meters)) {
        $account = [string]$meter.Account
        $meterId = [string]$meter.MeterId
        if ([string]::IsNullOrWhiteSpace($account) -or [string]::IsNullOrWhiteSpace($meterId)) { continue }
        $key = $account + '|' + $meterId
        $valid[$key] = ([string]$meter.MonthStatus -ne 'Передано')
    }

    $changed = $false
    foreach ($key in @($Drafts.Keys)) {
        if (-not $valid.ContainsKey([string]$key) -or -not [bool]$valid[[string]$key]) {
            [void]$Drafts.Remove([string]$key)
            $changed = $true
        }
    }
