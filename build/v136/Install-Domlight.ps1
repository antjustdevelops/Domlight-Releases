param(
    [string]$TargetRoot = (Join-Path $env:USERPROFILE 'Downloads\Domlight_v136_CLEAN'),
...
try {
    ...
    if([string]$manifest.version -ne 'v136 RELEASE'){throw ('Expected v136 RELEASE manifest, got: '+[string]$manifest.version)}
    ...
    $stage=Join-Path $env:TEMP ('Domlight_v136_'+[Guid]::NewGuid().ToString('N'))
    ...
    Set-Content -LiteralPath (Join-Path $stage 'VERSION.txt') -Value ('Domlight '+[string]$manifest.version) -Encoding UTF8
    ...
    Show-Info ("v136 CLEAN установлена и проверена.`r`n`r`nПапка:`r`n"+$TargetRoot+"`r`n`r`nСтарая тестовая папка не изменялась.")
